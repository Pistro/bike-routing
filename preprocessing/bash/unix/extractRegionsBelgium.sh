...
python3 osm2graph \
	...
	--json out=../source/output/$1_full.json \
...
osmosis \
  --read-xml ../source/inter/belgium_inter.osm \
  --bounding-box top=51.15 left=3.65 bottom=51 right=3.85 \
  --write-xml ../source/inter/ghent_inter.osm
createWgrAndJson ghent
  
osmosis \
  --read-xml ../source/inter/belgium_inter.osm \
  --bounding-box top=50.81 left=3.79 bottom=50.74 right=3.93 \
  --write-xml ../source/inter/gbergen_inter.osm
createWgrAndJson gbergen
  
osmosis \
  --read-xml ../source/inter/belgium_inter.osm \
  --bounding-box top=51.4 left=3.3 bottom=50.7 right=4.3 \
  --write-xml ../source/inter/east-flanders_inter.osm
createWgrAndJson east-flanders